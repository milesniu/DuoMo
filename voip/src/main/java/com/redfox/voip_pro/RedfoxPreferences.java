...
public class RedfoxPreferences {
	private static final int LINPHONE_CORE_RANDOM_PORT = -1;
	private static RedfoxPreferences instance;
	private Context mContext;

	public static final synchronized RedfoxPreferences instance() {
		if (instance == null) {
			instance = new RedfoxPreferences();
		}
		return instance;
	}

	private RedfoxPreferences() {
	...
	private String getString(int key) {
		if (mContext == null && RedfoxManager.isInstanciated()) {
			mContext = RedfoxManager.getInstance().getContext();
		}
	...
	private LinphoneCore getLc() {
		...
		return RedfoxManager.getLcIfManagerNotDestroyedOrNull();
	}
	...
	public LpConfig getConfig() {
		...
		if (!RedfoxManager.isInstanciated()) {
			Log.w("RedfoxManager not instanciated yet...");
			return LinphoneCoreFactory.instance().createLpConfig(mContext.getFilesDir().getAbsolutePath() + "/.linphonerc");
		}

		return LinphoneCoreFactory.instance().createLpConfig(RedfoxManager.getInstance().mLinphoneConfigFile);
	}
	...
	public void setAccountPassword(int n, String password) {
		if(getAccountDomain(n) != null && getAccountUsername(n) != null) {
			...
			RedfoxManager.getLc().addAuthInfo(authInfo);
		}
	}
	...
	public void setTunnelHost(String host) {
		...
		if(config != null) {
			config.setHost(host);
			RedfoxManager.getInstance().initTunnelFromConf();
		}
	}
