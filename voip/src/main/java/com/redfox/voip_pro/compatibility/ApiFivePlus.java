...
import com.redfox.voip_pro.RedfoxContact;
...
public class ApiFivePlus {
	...
	public static RedfoxContact getContact(ContentResolver cr, Cursor cursor, int position) {
		try {
			if(cursor != null) {
				cursor.moveToFirst();
	...
	public static Uri findUriPictureOfContactAndSetDisplayName(LinphoneAddress address, ContentResolver cr) {
		...
		if(cursor != null) {
			RedfoxContact contact = getContact(cr, cursor, 0);
...
//		NotificationCompat.Builder notifBuilder = new NotificationCompat.Builder(context)
//		.setSmallIcon(R.drawable.redfox_notification_icon)
//		.setContentTitle(title)
//		.setContentText(text)
//		.setContentIntent(intent);
//
