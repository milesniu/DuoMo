			...
			mProfileListener = new BluetoothProfile.ServiceListener() {
				...
				public void onServiceDisconnected(int profile) {
				    if (profile == BluetoothProfile.HEADSET) {
				        ...
				        RedfoxManager.getInstance().routeAudioToReceiver();
				    }
				}
			};
	...
	private boolean ensureInit() {
		...
		if (mContext == null) {
			if (RedfoxService.isReady()) {
				mContext = RedfoxService.instance().getApplicationContext();
			} else {
				return false;
			}
		}
...
//				RedfoxManager.getInstance().audioStateChanged(AudioState.BLUETOOTH);
    			isScoConnected = true;
        	} else if (state == AudioManager.SCO_AUDIO_STATE_DISCONNECTED) {
        		Log.d("Bluetooth sco state => disconnected");
//				RedfoxManager.getInstance().audioStateChanged(AudioState.SPEAKER);
