...
public final class RedfoxService extends Service {
	...
	private static RedfoxService instance;
	 ...
	 */
	public static RedfoxService instance()  {
		if (isReady()) return instance;

		throw new RuntimeException("RedfoxService not instantiated yet");
	}
...
/*		Intent notifIntent = new Intent(this, incomingReceivedActivity);
		...
		mNotif = Compatibility.createNotification(this, mNotificationTitle, "", R.drawable.redfox_notification_icon, R.mipmap.ic_launcher, bm, mNotifContentIntent, true,notifcationsPriority);

		RedfoxManager.createAndStart(RedfoxService.this);

		instance = this; // instance is ready once linphone manager has been created
		RedfoxManager.getLc().addListener(mListener = new LinphoneCoreListenerBase(){
			...
			public void callState(LinphoneCore lc, LinphoneCall call, State state, String message) {
				...
				if (state == State.CallUpdatedByRemote) {
					...
					if (remoteVideo && !localVideo && !autoAcceptCameraPolicy && !RedfoxManager.getLc().isInConference()) {
						try {
							RedfoxManager.getLc().deferCallUpdate(call);
				...
				if (state == State.StreamsRunning) {
					...
					if (getResources().getBoolean(R.bool.enable_call_notification))
						refreshIncallIcon(RedfoxManager.getLc().getCurrentCall());
				}
			}
...
//				}
				if (!mDisableRegistrationStatus) {
					if (state == RegistrationState.RegistrationOk && RedfoxManager.getLc().getDefaultProxyConfig() != null && RedfoxManager.getLc().getDefaultProxyConfig().isRegistered()) {
						sendNotification(IC_LEVEL_ORANGE, R.string.notification_registered);
					}
			
					if ((state == RegistrationState.RegistrationFailed || state == RegistrationState.RegistrationCleared) && (RedfoxManager.getLc().getDefaultProxyConfig() == null || !RedfoxManager.getLc().getDefaultProxyConfig().isRegistered())) {
	...
	private synchronized void setIncallIcon(IncallIconState state) {
		...
		RedfoxContact contact = ContactsManager.getInstance().findContactFromAddress(address);
	...
	public void displayMessageNotification(String fromSipUri, String fromName, String message) {
		...
		Uri pictureUri = null;
		try {
			RedfoxContact contact = ContactsManager.getInstance().findContactFromAddress(LinphoneCoreFactory.instance().createLinphoneAddress(fromSipUri));
			if (contact != null)
	...
	private Class<? extends Activity> incomingReceivedActivity = null;//LinphoneActivity.class;
	...
	public synchronized void sendNotification(int level, int textId) {
		...
		if (text.contains("%s") && RedfoxManager.getLc() != null) {
			// Test for null lc is to avoid a NPE when Android mess up badly with the String resources.
			LinphoneProxyConfig lpc = RedfoxManager.getLc().getDefaultProxyConfig();
	...
	public void onTaskRemoved(Intent rootIntent) {
		if (getResources().getBoolean(R.bool.kill_service_with_task_manager)) {
			...
			RedfoxManager.getLc().setNetworkReachable(false);
			stopSelf();
		}
