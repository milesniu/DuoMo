        ...
        switch (v.getId())
        {
            ...
            case R.id.bt_right:
                ...
                }else if (edit_actionnum.getText().toString().equals(""))
                {
                    ...
                    return;
                } else if (Integer.parseInt(edit_actionnum.getText().toString()) > 1000)
                {
                    MyLog.showToast(mContext, "应急队伍编号应在0~1000");
                    return;
                } else if (edit_actionname.getText().toString().equals(""))
                {
                    MyLog.showToast(mContext, "请输入命令名称");
                    return;
                }
                ...
                else if (Integer.parseInt(edit_steptime.getText().toString())>2592000)
                {
                    MyLog.showToast(mContext, "时间间隔应在0~2592000之间");
                    return;
                }else if (edit_targename.getText().toString().getBytes().length > 15)
                {
                    MyLog.showToast(mContext, "目的地名称不能超过15个字节");
                    return;
                } else if (edit_targeintro.getText().toString().getBytes().length > 49)
                {
