...
public class CreatcodeEarthActivity extends AbsCreatCodeActivity
{
	...
	public void onClick(View v)
	{
		...
		case R.id.bt_addoption:
			startActivityForResult(new Intent(mContext, CreatOptionActivity.class).putExtra("typecode", CreatOptionActivity.EARTHCODE), CreatOptionActivity.EARTHCODE);
			break;
		}
	}
	...
	{
		String data = "P1=0000&P2=001&P3="+skycode.get(sp_Code.getSelectedItemPosition()).get("code").toString()+(edit_name.getText().toString().equals("")?"":("&P4="+edit_name.getText().toString()))+"&P5="+skycolor.get(sp_Color.getSelectedItemPosition()).get("code").toString()+"&P6="+edit_time.getText().toString()+(options.size()>0?("&P7="+options.size()):"");
		String option = "";
		for(int i=0;i<options.size();i++)
		{
			...
			String P76x = item.get("lane").toString();
			String P77x = item.get("speed").toString();
			String P78x = item.get("step").toString();
			option+=("&P72"+(i+1)+"="+P72x+
					"&P73"+(i+1)+"="+P73x+
					"&P74"+(i+1)+"="+P74x+
					"&P75"+(i+1)+"="+P75x+
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		...
		if(requestCode == CreatOptionActivity.EARTHCODE&&data!=null)
		{
